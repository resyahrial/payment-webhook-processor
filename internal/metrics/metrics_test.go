...
func TestMetricsExportsPrometheusCompatibleSeries(t *testing.T) {
	...
	m.IncPaymentProcessing("updated")
	m.IncPaymentIgnored("older_timestamp")
	...
	assertCounterValue(t, m, "payment_webhook_payment_processing_total", map[string]string{"status": "updated"}, 1)
	assertCounterValue(t, m, "payment_webhook_payment_ignored_total", map[string]string{"reason": "older_timestamp"}, 1)
